...
if (
 ...
 const handleDelete = async () => {
  ...
  const openEditModal = (status: DefectStatus) => {
    ...
    setFormData({
      name: status.name || "",
      color: normalizeColor(status.color || "#000000"),
      type: status.type || "",
    });
                ...
                statusTypes.map((status) => (
                    <TableRow key={status.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center">
                          <div
                              className="w-6 h-6 rounded-full border border-gray-300 mr-3 shadow-sm"
                              style={{ backgroundColor: status.color || "#6B7280" }}
                          />
                          <span className="font-mono text-xs text-gray-700">{status.color || "None"}</span>
                        </div>
                      </TableCell>
