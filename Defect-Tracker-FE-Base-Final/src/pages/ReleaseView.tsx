  ...
  useEffect(() => {
    ...
    if (projects.length > 0) {
      if (!selectedProject) {
        ...
        console.log('Auto-selecting first project:', first);
        setSelectedProject(String(first.id));
    ...
    <div className="max-w-6xl mx-auto py-8">
      ...
      >
          <form onSubmit={async (e) => {
            e.preventDefault();
            if (!selectedProject) {
              showToast("Please select a project before creating a release", "error");
              return;
            }

            const projId = Number(selectedProject);
            if (!projId || isNaN(projId)) {
              showToast("Please select a valid project first", "error");
              return;
            }
            ...
            const relTypeId = releaseFormData.releaseType_id ? Number(releaseFormData.releaseType_id) : undefined;
            const payload = {
              name: releaseFormData.name.trim(),
              version: releaseFormData.version.trim() || "v1.0.0",
              releaseDate: releaseFormData.releaseDate, 
              releaseType_id: relTypeId,
              releaseTypeId: relTypeId,
              project_id: projId,
              projectId: projId,
              status: "PLANNED",
            };
