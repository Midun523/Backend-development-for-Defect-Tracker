...
export const ModuleManagement: React.FC = () => {
  ...
  const [roleMapByName, setRoleMapByName] = useState<Record<string, string>>({});
  const [roleMapById, setRoleMapById] = useState<Record<number, string>>({});

  const MODULE_ROLES = new Set(["QA_LEAD", "QA_ENGINEER"]);
  const SUBMODULE_ROLES = new Set([
    "DEVELOPER",
    "SENIOR_DEVELOPER",
    "DEV_LEAD",
    "JUNIOR_DEVELOPER",
  ...
  const getEffectiveRoleType = (dev: {
    userWithRole: string;
    role?: string;
    roleType?: string;
    roleId?: number;
    ...
    if (dev.roleType) {
      const normType = normalizeRoleName(dev.roleType);
      if (MODULE_ROLES.has(normType) || SUBMODULE_ROLES.has(normType)) {
        return normType;
      }
    }
  ...
  const getRoleTypedDevelopers = () => {
    return developersWithRoles.filter((dev) => {
      const effType = getEffectiveRoleType(dev);
      if (SUBMODULE_ROLES.has(effType)) {
        return true;
      }
  ...
  const fetchDevelopersWithRoles = async () => {
    ...
    try {
      ...
      try {
        const rolesRes = await getAllRoles(0, 100);
        const backendRoles = rolesRes.data?.content || [];
        backendRoles.forEach((r: any) => {
          if (r.type) {
            const normType = normalizeRoleName(r.type);
            byId[r.id] = normType;
            byName[normalizeRoleName(r.roleName || r.name)] = normType;
          }
        });
        setRoleMapByName(byName);
        setRoleMapById(byId);
      } catch (roleErr) {
        console.warn("Could not fetch backend roles for mapping:", roleErr);
      }

      const token = localStorage.getItem("authToken");
      const headers = token ? { Authorization: `Bearer ${token}` } : {};
      const res = await axios.get(`${ENDPOINTS.projectAllocation}/${selectedProjectId}`, { headers });
      const rawAllocations = res.data?.data || [];
      let allocations = Array.isArray(rawAllocations) ? rawAllocations : [];

      let activeAllocations = allocations.filter((a: any) => {
        const status = String(a.status || "ACTIVE").toUpperCase();
        return status !== "DEALLOCATED" && status !== "INACTIVE";
      });

      if (activeAllocations.length === 0) {
        try {
          const empRes = await axios.get(ENDPOINTS.projectAllocationProjectEmployee(Number(selectedProjectId)), { headers });
          const emps = empRes.data?.data || [];
          if (Array.isArray(emps) && emps.length > 0) {
            activeAllocations = emps.map((emp: any, idx: number) => ({
              id: emp.id || (idx + 1),
              employeeId: emp.id,
              userId: emp.id,
              userFullName: `${emp.firstName || ''} ${emp.lastName || ''}`.trim(),
              role: emp.designation?.designationName || emp.designation || 'Developer',
              status: 'ACTIVE',
              employee: emp,
            }));
          }
        } catch (e) {
          console.warn("Could not fetch project employees fallback:", e);
        }
      }

      const mapped = activeAllocations.map((alloc: any) => {
        const emp = alloc.employee || {};
        const user = emp.user || {};
        const employeeId = alloc.employeeId || alloc.userId || emp.id || user.id;
        const firstName = alloc.firstName || emp.firstName || user.firstName || "";
        const lastName = alloc.lastName || emp.lastName || user.lastName || "";
        const fullName = alloc.userFullName || `${firstName} ${lastName}`.trim() || `Employee ${employeeId}`;
        const role = alloc.role || alloc.roleName || user.roles?.[0]?.name || user.roles?.[0]?.roleName || user.roles?.[0]?.type || emp.designation?.designationName || emp.designation || "Developer";
        const roleType = alloc.roleType || user.roles?.[0]?.type || "";
        const roleId = alloc.roleId || user.roles?.[0]?.id || 1;
  ...
  const handleAddModule = async () => {
    if (moduleForm.name.trim() && selectedProjectId) {
      ...
      try {
        const response = await createModuleApi(Number(selectedProjectId), payload);
    ...
    if (isEditingSubmodule && editingSubmoduleId) {
      ...
      try {
        const response = await updateSubmoduleApi(
          ...
          Number(editingSubmoduleId),
          {
            ...
            subModuleName: submoduleForm.name.trim(),
            submoduleName: submoduleForm.name.trim(),
          },
        );
      ...
      try {
        const payload = {
          ...
          module_id: Number(currentModuleIdForSubmodule),
        };
        const response = await createSubmodule(
          Number(currentModuleIdForSubmodule),
          payload,
        );
        console.log("Create submodule response:", response);
  ...
  const handleBulkAssignment = (itemsToAssign?: Array<{ type: "module" | "submodule"; moduleId: string; submoduleId?: string }>) => {
    fetchDevelopersWithRoles();
    const items = itemsToAssign || selectedItems;
    if (items && items.length > 0) {
      if (itemsToAssign) {
        setSelectedItems(itemsToAssign);
      }
      setAssignmentForm({
        moduleId: "",
        employeeIds: [],
  ...
  const fetchModules = async () => {
    ...
    setIsLoading(true);
    try {
      const response: any = await getModulesByProjectId(Number(selectedProjectId));
      const rawList = response?.data || (Array.isArray(response) ? response : []);
      if (Array.isArray(rawList)) {
        const modulesWithSubmodules = await Promise.all(
          rawList.map(async (module: any) => {
            try {
              const submodulesRes: any = await getSubmodulesByModule(module.id);
              const subList =
                submodulesRes?.data ||
                (Array.isArray(submodulesRes) ? submodulesRes : []);
              return {
                ...module,
                name: module.name || module.moduleName,
                moduleName: module.moduleName || module.name,
                submodules: Array.isArray(subList) ? subList : [],
              };
            } catch (err) {
              console.error(
  ...
  useEffect(() => {
    const fetchDeveloperRoles = async () => {
      ...
      try {
        ...
        setDeveloperRoleNames([...new Set([...defaultNames, ...roleNameGroups.flat()])]);
        setDeveloperRoleIds(roleIdGroups.flat());
      } catch (error) {
        console.error("Error fetching developer roles:", error);
        setDeveloperRoleNames(defaultNames);
        setDeveloperRoleIds([]);
      }
    };
  ...
  useEffect(() => {
    const fetchModuleLeaderRoles = async () => {
      ...
      try {
        ...
        setModuleLeaderRoleNames([...new Set([...defaultNames, ...roleNameGroups.flat()])]);
        setModuleLeaderRoleIds(roleIdGroups.flat());
      } catch (error) {
        console.error("Error fetching module leader roles:", error);
        setModuleLeaderRoleNames(defaultNames);
                ...
                {modulesByProjectId &&
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch">
                    {(filteredModules || []).map((module) => {
                      ...
                      const leaderUser = module.leader || module.assignedDev;
                      const leaderName = leaderUser
                        ? (leaderUser.userName || leaderUser.userFullName || `${leaderUser.firstName || ''} ${leaderUser.lastName || ''}`.trim())
                        : null;
                      const moduleDevs = leaderName
                        ? [{ userName: leaderName, userId: leaderUser.id || leaderUser.userId }]
                        : [];
                      console.log(
                ...
                <div className="max-h-60 overflow-y-auto space-y-2">
                  {getRoleTypedModuleLeaders().length > 0 ? (
                    getRoleTypedModuleLeaders()
                      .filter(dev => {
                        const selectedModules = modulesByProjectId?.filter(m =>
                          selectedItems.some(i => i.type === "module" && String(i.moduleId) === String(m.id))
                        ) || [];
                        const isAlreadyLeader = selectedModules.some(m => {
                          const leaderId = m.leader?.id || (m as any).leaderId;
                          return leaderId && Number(leaderId) === Number(dev.userId);
                        });
                        return !isAlreadyLeader;
                      })
                      .map((dev, idx) => {
                ...
                <div className="max-h-60 overflow-y-auto space-y-2">
                  {moduleAllocatedUsersLoading ? (
                    ...
                    moduleAllocatedUsers.map((user, idx) => {
                      const userId = user.userId || user.id || user.employeeId;
                      const rawName = user.name || `${user.firstName || ''} ${user.lastName || ''}`.trim() || `User ${userId}`;
                      const rawRole = user.roleName || user.role || user.designation?.designationName || user.designation || "";
                      const [name, role] = user.userWithRole && user.userWithRole.includes("-")
